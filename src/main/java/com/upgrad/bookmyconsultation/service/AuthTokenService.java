...
//		}
		...
		final List<UserAuthToken> userAuthTokens = userAuthDao.findByUserEmailId(user.getEmailId());

		for (UserAuthToken token : userAuthTokens) {
			final UserAuthTokenVerifier tokenVerifier = new UserAuthTokenVerifier(token);
			if (tokenVerifier.isActive()) {
				// If you find an active token, you could return it
				// Or, if you prefer, update its expiry and return it
				token.setExpiresAt(now.plusHours(8));
				userAuthDao.save(token);
				return token;
			} else {
				// If the token is not active, delete it
				userAuthDao.delete(token);
			}
		}

		// If you get to this point, no active tokens were found, so create a new one
